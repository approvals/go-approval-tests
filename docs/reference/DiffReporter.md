...
1. **[IntelliJ Reporter](../how_to/UseIntelliJReporter.md)** — detects any running JetBrains IDE automatically (cross-platform)
2. **Diff tool on macOS** — tries installed Mac apps in this order:
...
3. **Diff tool on Windows** — tries installed Windows apps in this order:
...
4. **Diff tool on Linux** — tries installed Linux tools in this order:
   - DiffMerge, Meld, KDiff3, Sublime Merge, `diff` (command line), Delta, Zed
5. **Quiet reporter** — silently does nothing if no tool is found
6. **Print supported programs** — lists the programs that are supported so you know what to install
